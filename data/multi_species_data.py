...
class ExperimentDatasetMultiEqual(data.Dataset):
    def __init__(
        ...
        self.total_num_cells = self.max_cells * len(
            self.species
        )  # number of cells * number of species
...
def multi_species_collate_fn(
    ...
    for species in all_species:
        if has_batch_labels:
            data, labels, refs, batch_labels = (
                torch.stack(species_to_data[species]),
                torch.stack(species_to_labels[species]),
                torch.stack(species_to_refs[species]),
                torch.stack(species_to_batch_labels[species]),
            )
        else:
            data, labels, refs, batch_labels = (
                torch.stack(species_to_data[species]),
...
class ExperimentDatasetMultiEqualCT(data.Dataset):
    def __init__(
        self, all_data: dict, ys_col: str, refs_col: str, cluster_col: str
    ) -> None:
        ...
        for species, data in all_data.items():
            ...
            adatas = [
                data[data.obs[cluster_col].isin([clust])]
                for clust in data.obs[cluster_col].cat.categories
            ]
        ...
        all_ys = {
            species: adata.obs[ys_col]
            for (species, adata) in species_to_new_adata.items()
        }
        all_refs = {
            species: adata.obs[refs_col]
            for (species, adata) in species_to_new_adata.items()
        }
