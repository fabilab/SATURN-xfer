...
from model.xfer_model import TransferModel
...
def train(
    ...
    for batch_idx, batch_dict in enumerate(train_loader):
        ...
        for species, (data, labels, ref_labels, _) in batch_dict.items():
            if data is None:
                continue
            data, labels, ref_labels = (
                data.to(device),
                labels.to(device),
                ref_labels.to(device),
            )
            ...
            spec.append(
                np.argmax(np.array(sorted_species_names) == species)
        ...
        if use_ref_labels:
            indices_tuple = mining_func(
                embeddings, labels, species, mnn=mnn, ref_labels=ref_labels
            )
        else:
            indices_tuple = mining_func(embeddings, labels, species, mnn=mnn)

        indices_mapped = [
            labels[i] for i in indices_tuple
        ]  # map to labels for only the purpose of writing to triplets file
        for j in range(len(indices_mapped[0])):
            key = (
                f"{indices_mapped[0][j]},{indices_mapped[1][j]},{indices_mapped[2][j]}"
            )
        ...
        if equalize_triplets_species:
            species_mapped = [
                species[i] for i in indices_tuple
            ]  # a,p,n species vectors
            a_spec = species_mapped[0]
            p_spec = species_mapped[1]
            a_uq, a_inv, a_ct = torch.unique(
                a_spec, return_counts=True, return_inverse=True
            )
            p_uq, p_inv, p_ct = torch.unique(
                p_spec, return_counts=True, return_inverse=True
            )

            a_prop = a_ct / torch.sum(a_ct)  # Proportions of total ie 1/4
            p_prop = p_ct / torch.sum(p_ct)  # Proportions of total ie 3/4

            a_balance = torch.reciprocal(a_prop) / len(
                a_prop
            )  # balancing ie * 4 / 1, then divide by num species
            p_balance = torch.reciprocal(p_prop) / len(
                p_prop
            )  # balancing ie 4 / 3, then divide by num species
...
def get_all_embeddings(dataset, model, device, use_batch_labels=False, obs_names=None):
        ...
        for batch_idx, batch_dict in tqdm(
            ...
            for species, (data, labels, ref_labels, batch_labels) in batch_dict.items():
                if data is None:
                    continue
                if use_batch_labels:
                    data, labels, ref_labels, batch_labels = (
                        data.to(device),
                        labels.to(device),
                        ref_labels.to(device),
                        batch_labels.to(device),
                    )
                    encoder_inputs, encodeds, mus, log_var, px_rate, px_r, px_drop = (
                        model(data, species, batch_labels)
                    )
                else:
                    data, labels, ref_labels = (
                        data.to(device),
    ...
    if use_batch_labels:
        return (
            torch.stack(embs).cpu().numpy(),
            np.array(labs),
            np.array(spec),
            torch.stack(macrogenes),
            np.array(refs),
            np.array(batch_labs),
        )
    else:
...
def create_output_anndata(
    ...
    adata.obs["labels"] = pd.Categorical([id2cell_type[int(l)] for l in labels])
    ...
    adata.obs["ref_labels"] = pd.Categorical([id2ref_type[int(l)] for l in ref_labels])
    ...
    if use_batch_labels:
        ...
        adata.obs["batch_labels"] = pd.Categorical(
            [id2batch_type[int(l)] for l in batch_labels]
        )
...
def inferrer(args):
    ...
    species_to_adata = {
        species: sc.read(path) for species, path in species_to_path.items()
    }
    ...
    if args.tissue_subset is not None:
        for species in species_to_adata.keys():
            ...
            species_to_adata[species] = ad[
                ad.obs[args.tissue_column] == args.tissue_subset
            ]
            if species_to_adata[species].X.shape[0] == 0:
                raise ValueError(
                    f"No cells in {args.tissue_subset} found in the input dataset"
                )
    ...
    if args.in_label_col is None:
        ...
        species_to_label_col = {
            species: col for species, col in species_to_label.items()
        }
    ...
    for adata in species_to_adata.values():
        adata.obs["truth_labels"] = pd.Categorical(
            values=[
                celltype_id_map[cell_type]
                for cell_type in adata.obs["species_type_label"]
            ]
        )
    ...
    if use_batch_labels:
        ...
        for adata in species_to_adata.values():
            unique_batch_types = unique_batch_types | set(
                adata.obs[args.non_species_batch_col]
            )

        unique_batch_types = sorted(unique_batch_types)
        batchtype_id_map = {
            batch_type: index for index, batch_type in enumerate(unique_batch_types)
        }
        for adata in species_to_adata.values():
            adata.obs["batch_labels"] = pd.Categorical(
                values=[
                    batchtype_id_map[batch_type]
                    for batch_type in adata.obs[args.non_species_batch_col]
                ]
            )
    ...
    reftype_id_map = {
        ref_type: index for index, ref_type in enumerate(unique_ref_types)
    }
    for adata in species_to_adata.values():
        adata.obs["ref_labels"] = pd.Categorical(
            values=[
                reftype_id_map[ref_type] for ref_type in adata.obs[args.ref_label_col]
            ]
        )
    ...
    for species in sorted_species_names:
        ...
        if use_batch_labels:
            sc.pp.highly_variable_genes(
                adata,
                flavor="seurat_v3",
                n_top_genes=args.hv_genes,
                batch_key=args.non_species_batch_col,
                span=args.hv_span,
            )  # Expects Count Data
        else:
            sc.pp.highly_variable_genes(
        ...
        species_to_gene_embeddings[species] = species_to_gene_embeddings[species][
            hv_index
        ]
        species_to_gene_idx_hv[species] = (
            ct,
    ...
    # stacked embeddings
    X = torch.cat(
        [species_to_gene_embeddings[species] for species in sorted_species_names]
    )
    ...
    if (args.guide_species is not None) or args.train:
        if (args.guide_species is not None) and (args.guide_species != "auto"):
            ...
            centroid_weights_trained = torch.stack(
                [
                    torch.tensor(species_genes_scores_trained[gn])
                    for gn in all_gene_names_trained
                ]
            )
            metric = {}
            for species in sorted_species_names_trained:
                centroid_weights_trained_species = torch.stack(
                    [
                        torch.tensor(species_genes_scores_trained[gn])
                        for gn in all_gene_names_trained
                        if gn.startswith(species)
                    ],
                )
                cdist_species = torch.cdist(
                    centroid_weights, centroid_weights_trained_species
                )
            ...
            del (
                metric,
                centroid_weights_trained_species,
                cdist_species,
                dis_closest,
                nconserved,
                dis_mean,
        ...
        tmp = "on" if args.project_to_guide else "off"
        print(
            f"Guide species within the training set: {species_closest}. Projection onto guide is {tmp}."
        )
        ...
        # the xfer_model.
        if args.project_to_guide:
            print(
                "Connect genes of new species with genes of closest species for inference"
            )
            hvgs_guide_species = [
                gn for gn in all_gene_names_trained if gn.startswith(species_closest)
            ]
            # Strip the "species-" part in front of each gene name
            hvgs_guide_plain = [
                x[len(species_closest) + 1 :] for x in hvgs_guide_species
            ]
            ...
            embedding_path_closest = pd.read_csv(
                args.training_csv_path, index_col="species"
            ).at[species_closest, "embedding_path"]
            ...
            gene_embeddings_closest_hvg = torch.stack(
                [gene_embedding_dict_guide[gn] for gn in hvgs_guide_plain]
            )
            ...
            matrix_match_gene_guides_norm = (
                matrix_match_gene_guides.T / matrix_match_gene_guides.sum(axis=1)
            ).T
    ...
    xfer_state_dict = {}
    if not args.project_to_guide:
        # NOTE: this also sends them to the device
        xfer_state_dict["p_weights"] = centroid_weights.T.type(
            pretrain_state_dict["p_weights"].type()
        )
        if args.random_weights:
            xfer_state_dict["p_weights"] = torch.rand_like(xfer_state_dict["p_weights"])
        ...
        xfer_state_dict["guide_weights"] = torch.tensor(
            matrix_match_gene_guides_norm.T.astype(np.float32)
        ).to(device)
        if args.random_weights:
            xfer_state_dict["guide_weights"] = torch.rand_like(
                xfer_state_dict["guide_weights"]
            )
        # leave an open multiplier for the guide gene space, decide later whether to freeze it or not
        xfer_state_dict["guide_layer_norm.weight"] = torch.ones(
            matrix_match_gene_guides_norm.shape[1]
        ).to(device)
        xfer_state_dict["guide_layer_norm.bias"] = torch.zeros(
            matrix_match_gene_guides_norm.shape[1]
        ).to(
            device
    ...
    xfer_state_dict["cl_layer_norm.weight"] = deepcopy(
        pretrain_state_dict["cl_layer_norm.weight"]
    )
    xfer_state_dict["cl_layer_norm.bias"] = deepcopy(
    ...
    xfer_model = TransferModel(
        ...
        guide_species=args.guide_species if args.project_to_guide else None,
        species_to_gene_idx=(
            species_to_gene_idx_hv_trained if args.guide_species is not None else {}
        ),
        vae=args.vae,
        dropout=0.1,
    )
    ...
    if species_closest is not None:
        adata_zeroshot.uns["guide_species"] = species_closest
    ...
    if args.trained_adata_path is None:
        raise ValueError(
            "Must provide a trained adata path for training the transfer model."
        )
    ...
    for adata in species_to_adata_xfer.values():
        unique_cell_types_xfer = unique_cell_types_xfer | set(
            adata.obs["species_type_label"]
        )
    unique_cell_types_xfer = sorted(unique_cell_types_xfer)
    celltype_id_map_xfer = {
        cell_type: index for index, cell_type in enumerate(unique_cell_types_xfer)
    }
    for adata in species_to_adata_xfer.values():
        adata.obs["truth_labels"] = pd.Categorical(
            values=[
                celltype_id_map_xfer[cell_type]
                for cell_type in adata.obs["species_type_label"]
            ]
        )
    ...
    for adata in species_to_adata_xfer.values():
        unique_ref_types_xfer = unique_ref_types_xfer | set(
            adata.obs[args.ref_label_col]
        )
    unique_ref_types_xfer = sorted(unique_ref_types_xfer)
    reftype_id_map_xfer = {
        ref_type: index for index, ref_type in enumerate(unique_ref_types_xfer)
    }
    for adata in species_to_adata_xfer.values():
        adata.obs["ref_labels"] = pd.Categorical(
            values=[
                reftype_id_map_xfer[ref_type]
                for ref_type in adata.obs[args.ref_label_col]
            ]
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_df = pd.DataFrame.from_records(
            list(epoch_indices_counts.items()), columns=["Triplet", "Count"]
        )
    ...
    if species_closest is not None:
        adata_finetuned.uns["guide_species"] = species_closest
    adata_finetuned.uns["xfer_training_epoch"] = args.epochs
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--guide_species",
        type=str,
        help="Choose a guide species to project gene expression onto. Default is to take the one with the closest embeddings for conserved proteins.",
    )
    ...
    parser.add_argument(
        "--embedding_model",
        type=str,
        choices=["ESM1b", "MSA1b", "protXL", "ESM1b_protref", "ESM2"],
    ...
    parser.add_argument(
        "--project_to_guide",
        action="store_true",
        help="Project the new species onto the guide species first.",
    )
