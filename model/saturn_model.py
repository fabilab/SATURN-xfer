...
class SATURNPretrainModel(torch.nn.Module):
    def __init__(
        ...
        self.px_decoder = nn.Sequential(
            full_block(
                self.embed_dim + self.num_species + self.num_batch_labels,
                self.hidden_dim,
                self.dropout,
            ),
        )
    ...
    def forward(self, inp, species, batch_labels=None):
        ...
        if self.num_batch_labels > 0:
            ...
            batch_idx = np.argmax(
                np.array(self.sorted_batch_labels_names) == batch_labels
            )
            batch_1h[:, batch_idx] = 1.0
...
def make_centroids(
    ...
    print("Making Centroids using KMeans and scoring genes against them")
    if normalize:
    ...
    if kmeans is not None:
        try:
            print(f"Attempting pytorch-kmeans on {device}")
            ...
            print("pytorch-kmeans succeeded")
        except RuntimeError:
            print("pytorch-kmeans failed (memory?), falling back onto sklearn")
    ...
    if score_function == "default":
        ...
        raise ValueError(
            "score_function must be one of 'default', 'one_hot', or 'smoothed'"
        )
...
def score_genes_against_centroids(
    embeds,
    cluster_centers,
    species_gene_names,
    score_function="default",
...
def one_hot_centroids_scores(dd):
    ...
    to_scores = (ranked == 1).astype(
        float
    )  # true, which is rank 1, is highest, everything else is 0
    return to_scores
...
class OrthologPretrainModel(torch.nn.Module):
    def __init__(
        ...
        self.px_decoder = nn.Sequential(
            full_block(
                self.embed_dim + self.num_species, self.hidden_dim, self.dropout
            ),
        )
